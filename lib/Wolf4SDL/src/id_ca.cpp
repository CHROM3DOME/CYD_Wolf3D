...
#ifdef WOLF3D_CYD_PORT
        if(!cydMapScratch)
            Quit("Map scratch was not reserved");
        if(expanded > (int32_t) ((maparea + 1) * sizeof(word)))
            Quit("Map scratch too small: %i > %i", expanded, (maparea + 1) * sizeof(word));
        buffer2seg = cydMapScratch;
#else
