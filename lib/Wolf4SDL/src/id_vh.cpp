...
#ifdef WOLF3D_CYD_PORT
    CA_CacheGrChunk(picnum);
    VWB_DrawPicScaledCoord(scx * 8, scy, picnum);
    UNCACHEGRCHUNK(picnum);
#else
...
#ifdef WOLF3D_CYD_PORT
void LatchDrawChar (int x, int y, int picnum)
{
    CA_CacheGrChunk(STARTTILE8);
    VL_MemToScreen(grsegs[STARTTILE8] + picnum * 64, 8, 8, x, y);
}

void LatchDrawTile (int x, int y, int picnum)
{
