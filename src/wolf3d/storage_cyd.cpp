...
extern "C" int32_t wolf_read(int fd, void *buffer, size_t count) { return fd > 0 && fd < maxFiles && descriptors[fd] ? descriptors[fd].read(static_cast<uint8_t *>(buffer), count) : -1; }
extern "C" int32_t wolf_write(int fd, const void *buffer, size_t count) { return fd > 0 && fd < maxFiles && descriptors[fd] ? descriptors[fd].write(static_cast<const uint8_t *>(buffer), count) : -1; }
...
extern "C" int wolf_stat(const char *path, void *) { return pathExistsByListing(resolveExistingPath(path)) ? 0 : -1; }
extern "C" int wolf_unlink(const char *path) { return SD.remove(gamePath(path)) ? 0 : -1; }
...
extern "C" int wolf_fclose(WolfFile *file) { if (!file) return -1; file->file.close(); delete file; return 0; }
extern "C" size_t wolf_fread(void *buffer, size_t size, size_t count, WolfFile *file) {
  if (!file || !size) return 0; size_t bytes = file->file.read(static_cast<uint8_t *>(buffer), size * count); file->eof = bytes < size * count; return bytes / size;
}
extern "C" size_t wolf_fwrite(const void *buffer, size_t size, size_t count, WolfFile *file) { return file && size ? file->file.write(static_cast<const uint8_t *>(buffer), size * count) / size : 0; }
extern "C" int wolf_fseek(WolfFile *file, long offset, int whence) {
  if (!file) return -1; long target=offset; if(whence==SEEK_CUR) target+=file->file.position(); else if(whence==SEEK_END) target+=file->file.size(); file->eof=false; return file->file.seek(target, SeekSet) ? 0 : -1;
}
extern "C" long wolf_ftell(WolfFile *file) { return file ? file->file.position() : -1; }
extern "C" int wolf_feof(WolfFile *file) { return file && file->eof; }
